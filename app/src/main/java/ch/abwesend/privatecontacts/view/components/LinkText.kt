...
/**
 * A text-field showing the text as a link
 * See https://github.com/firefinchdev/linkify-text
 */
@Composable
fun LinkText(
    text: String,
    modifier: Modifier = Modifier,
    entireTextAsLink: Boolean = true,
    ...
    onLinkClicked: ((linkText: String) -> Unit)? = null
) {
    val uriHandler = LocalUriHandler.current
    val linkInfos =
        if (entireTextAsLink) listOf(LinkInfo(text, 0, text.length))
        ...
        ClickableText(
            ...
            onClick = { offset ->
                annotatedString.getStringAnnotations(
                    ...
                    if (entireTextAsLink) {
                        onLinkClicked?.invoke(annotatedString.substring(result.start, result.end))
                    } else {
